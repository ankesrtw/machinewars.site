...
**Phase 2 opened 2026-08-19 — P2.2 done, P2.1 scaffolded (device gate open).** The
Unity repo is real and the importer round-trips both directions; see "Next session"
for exactly what was verified and what comes next. The rest of this section is the
still-current Phase 1.9 scope context — **read it before any `data/` work.**
...
**Phase 2 is under way. P2.2 is DONE; P2.1 is scaffolded but its device gate is open.**

**The Unity repo now exists:** `d:/ai-projects/machinewars-unity` — Unity 6000.5.8f1,
URP 17.5, a **fresh repo, not a fork**, 2 commits. `data/json/` is checked in there at
`Assets/Data/json/` (30 files + `manifest.json`).

**What actually works today, verified headless (not just written):**
- `Machine Wars > Import Data` (or `-executeMethod
  MachineWars.EditorTools.DataImporter.ImportCLI`) imports 3 weapons, 5 enemy types,
  6 wave sets, 11 scenes, 10 missions, 1 campaign.
- The **staleness guard refuses** on any mismatch and writes **nothing partial** —
  tested with a hand-edited file, a deleted file, a genuine `data/` round-trip, and a
  CRLF checkout (which must *not* false-alarm, and does not).
- Re-import is **idempotent**; two bugs that only showed on the second import were
  found and fixed (see the log entry).

**Next, in order:**
1. **P2.3 — acquire the test device.** This is now the critical path: it gates P2.1's
   done-when *and* all of P2.5. Nothing else in Phase 2 can close without it.
   A ~3-year-old ~$250 phone (Snapdragon 6-series / Dimensity 900 class) — **not the
   editor, not a flagship.**
2. **P2.4 — build `warzone` from `SceneConfigSO`.** Everything it needs is imported and
   sitting in `Assets/ScriptableObjects/Scenes/warzone.asset` already: ground,
   `coverBlocks` (23 of them, as 6-tuples `[x,z,hw,hd,h,ry]`), `sceneAssets` GLB
   placements, sky/lighting palette. Load GLBs with **glTFast at edit time**, never
   runtime glTF on mobile. `background`/`sandParticles`/`propColors` were carried
   across as raw JSON strings on the SO (their shape varies per scene) — parse them at
   point of use.
3. **P2.5 — profile, write `docs/v2/perf-baseline.md`.**

**Rules that bind the Unity side, and are not negotiable:**
- Never hand-edit a generated ScriptableObject, and never hand-edit the Unity-side
  `data/json/` copy to silence the guard. Fix `data/` here, re-export, re-copy.
- Objective types are a hard cap of 11 shared with `data/objectives.schema.md`. The
  importer **throws** on an unknown type rather than skipping it — so the two lists
  cannot drift apart quietly.

**Gotcha that cost a build cycle:** Input System must be pinned to **1.20.0**. The
1.11.2 tried first does not compile against this editor (deprecated `TreeView` API), and
it fails as ~200 errors inside `PackageCache`, which looks alarming but is not your code.

**Concept art — no longer blocked on API spend.** The user opted to generate it manually
through GPT/Gemini rather than paying for `tools/gen-art.mjs`. Prompts for the two sites
that have **no art at all** are written up in
[art-prompts-ocean-grid.md](../../art-prompts-ocean-grid.md) — both `ocean` and `grid`
already reference `assets/art/zones/{ocean,grid}.jpg`, which do not exist yet. Dropping
those two files in needs **no `data/` edit**.

**Correction worth carrying:** the campaign graph is **10 nodes, not 11.** P1.9.1
retired `ghats`/`ghats_east`; several places in these docs still say 11. The importer
reads all 10 correctly — it is the prose that is stale.

**Untouched by the re-plan:** `data/`'s content-authoring contract (P0), `save.js` v2,
the campaign graph *mechanism*, and the mission schema.
...
- **2026-08-19** — **Phase 2 opened: P2.1 scaffold + P2.2 importer.** Created
  `machinewars-unity` (Unity 6000.5.8f1, URP 17.5, **fresh repo, not a fork** — a fork
  would carry `src/`/`play/`/`vendor/three/` into a repo that never runs them). Web
  side: `tools/data-to-json.mjs` now emits `data/json/manifest.json`, a sha256 over
  path + bytes of all 30 files, CRLF-normalized, **no timestamp** so it changes iff the
  data changes. Unity side: `DataImporter.cs` + `ManifestGuard.cs` + `MiniJson.cs`
  (hand-rolled — `JsonUtility` cannot express top-level arrays, string-keyed maps, or
  heterogeneous tuple arrays like `coverBlocks`) and the SO definitions including all
  **11** `ObjectiveSO` subclasses. Imports 3 weapons / 5 enemy types / 6 wave sets /
  11 scenes / 10 missions / 1 campaign. **P2.2's done-when verified in both directions
  headless:** a real `data/` edit (heavy hp 6→7) round-trips with zero hand-editing;
  a hand-edited copy, a deleted file, and a stale manifest are each **refused** by name
  with **nothing partial written** (`heavy.hp` stayed 6, not the injected 99); CRLF
  hashes identically so a Windows checkout raises no false refusal. **Two re-import
  bugs found and fixed**, both from AssetDatabase calls inside a `StartAssetEditing`
  batch not seeing that batch's own writes: numbered duplicate output folders, and
  mission objectives **doubling** on re-import (m001 2→4 — a mission that would play
  every objective twice). Three consecutive imports now stable. **P2.1 left unticked**
  — its done-when is an Android build+run and there is no device yet (P2.3 is now the
  critical path). Gotcha: Input System must be **1.20.0**, not 1.11.2. Also wrote
  `art-prompts-ocean-grid.md` (user is generating art manually via GPT/Gemini rather
  than paying for `gen-art.mjs`). Noted that the campaign graph is **10 nodes, not 11**
  as several docs still claim — P1.9.1 retired `ghats`/`ghats_east`.
