...
`tools/validate-data.mjs` are now the *only* guards on content validity — keep
them green.
...
- `missions/<id>.data.js` — one mission per implemented campaign node, plus
  ...
  (Unity implements the mission runner); `tools/validate-data.mjs` checks
  shape against the vocabulary and the campaign graph, plus cross-file
  references across the rest of `data/` (scene `waveSet`, wave-set enemy
  types, campaign node `site`s, and graph acyclicity).
...
- `json/manifest.json` — also generated, and the thing that keeps the Unity
  copy honest. It is a sha256 over path + bytes of all 30 emitted files
  (CRLF-normalized, and deliberately carrying **no timestamp**, so it changes
  **iff the data changes** — same discipline as `gen-version.mjs`'s
  `BUILD_ID`). `data/json/` is a **checked-in copy** in the Unity repo, and
  copies drift silently; the Unity importer re-hashes what it sees and
  **refuses to import** on a mismatch. **Refusing is the point** — a partial
  import is what tempts hand-patching the Unity side. When it rejects
  something, fix `data/` and re-export; never hand-edit the Unity copy.

## The Unity project (separate repo)

The actual game ships from **`machinewars-unity`** — a *fresh* Unity 6000.5.8f1 / URP
repo at `d:/ai-projects/machinewars-unity`, **not a fork** of this one (a fork would
drag `src/`, `play/` and `vendor/three/` into a repo that never runs them). Scaffolded
2026-08-19 as ROADMAP-V2 P2.1/P2.2.

- **The importer is the contract.** `Assets/Editor/DataImporter.cs` (`Machine Wars >
  Import Data`, or `-executeMethod MachineWars.EditorTools.DataImporter.ImportCLI` in
  batchmode) reads `Assets/Data/json/` — the checked-in copy of this repo's
  `data/json/` — into ScriptableObjects. It bakes at **edit time**, so bad data fails
  on a dev machine rather than reaching a player as a crash.
- **Updating it is one deliberate step:** run `node tools/data-to-json.mjs` here, copy
  `data/json/` across, commit both. The manifest hash tells you unambiguously whether
  you did — see the `json/manifest.json` note above.
- **Never hand-edit a generated ScriptableObject**, and never hand-edit the Unity-side
  `data/json/` copy to make the guard pass. If a Unity-only concept cannot be expressed
  in `data/`, that is a schema gap to close **in `data/`**.
- Objective types are a **hard cap of 11** shared with `data/objectives.schema.md`; the
  importer **throws** on an unknown type rather than skipping it.
- Gotcha: Input System must be **1.20.0** — 1.11.2 does not compile against this
  editor.
